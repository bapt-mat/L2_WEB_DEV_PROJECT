...
if (isset($_POST['content1']) AND !empty($_POST['content1']) AND isset($_POST['title']) AND !empty($_POST['title']) AND isset($_POST['id_cours']) AND !empty($_POST['id_cours'])){
    ...
    //recuperation du nombre de like du cours

    $bdd_cours = connexion('site');
    try{
        $recupNbLikes = $bdd_cours->prepare('SELECT nb_likes FROM cours WHERE id = :id');
        $recupNbLikes->bindParam(':id', $id_cours);
        $recupNbLikes->execute();
        $nb_likes = $recupNbLikes->fetch();
        $nb_likes = $nb_likes['nb_likes'];

        $recupNbLikes->closeCursor();
        $bdd_cours = null;
    }
    ...
    try{
        ...
        $new_id_cours = $bdd_cours->lastInsertId();
    ...
    //Ajout des parties 2 et 3 si elles existent
    ...
    if (isset($_POST['image']) AND !empty($_POST['image'])){
        $cheminImage = htmlspecialchars($_POST['image']);
        $cheminImage = str_replace('./bdd_cours/', './', $cheminImage);
        $coursPHP .= '
    ...
    if ((isset($_SESSION[\'pseudo\']) AND $_SESSION[\'pseudo\'] == \'admin\') OR ($_SESSION[\'id\'] == $id_auteur)){
        ...
        <form action="../modifier_cours.php" method="post" class="modif">
            ...
            <input type="hidden" name="id_cours" value="' . $new_id_cours . '">
    ...
    header('Location: ./bdd_cours/' . $title . '.php'); 
}
